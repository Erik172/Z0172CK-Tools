...
default_wl_pass="modules/FB-BruteForce/passwords.lst"
...
if [[ ! -d modules/FB-BruteForce/sessions ]]; then
mkdir modules/FB-BruteForce/sessions
fi
cum=$(grep -n "$PASS" "$wl_pass" | cut -d ":" -f1)
printf "EMAIL=\"%s\"\nPASS=\"%s\"\nwl_pass=\"%s\"\ncum=\"%s\"\ntoken=\"%s\"\n" $EMAIL $PASS $wl_pass $token $token > modules/FB-BruteForce/sessions/store.session.$EMAIL.$(date +"%FT%H%M")
printf "\e[1;77mSession saved.\e[0m\n"
printf "\e[1;92mUse [11] resume\n"
else
exit 1
fi
...
for list in $(ls modules/FB-BruteForce/sessions/store.session*); do
IFS=$'\n'
source $list
...
source $(ls modules/FB-BruteForce/sessions/store.session* | sed ''$fileresume'q;d')
